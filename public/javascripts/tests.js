...
var descriptions = [
	{text: "5x400. ended up doing 78, 81, 81, 81, 80 w/ 200 jog between.",
		expected: [{
				description: "5x400",
				n: 5,
				distance: 400,
				units: undefined,
				times: [78,81,81,81,80]
			}]
	},
	{text: "test test 4x200m in 32, 34, 33, 31. test test 3x1600 test in 4:56, 5:01, 4:58. Felt good.",
		expected: [
			{
				description: "4x200m",
				n: 4,
...
QUnit.test("parses description", function(assert) {
	var result;
	descriptions.forEach(function(io) {
	    result = parse_log(io.text);
		assert.ok(object_compare(result, io.expected));
	});
});
